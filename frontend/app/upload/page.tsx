...
export default function UploadPage() {
  ...
  async function uploadWithPresignedUrl(
    ...
    return new Promise<UploadedFile>((resolve, reject) => {
      ...
      const hasContentTypeHeader = presigned.headers
        ? Object.keys(presigned.headers).some((key) => key.toLowerCase() === 'content-type')
        : false;
      ...
      xhr.onload = () => {
        ...
        if (isSuccess) {
          resolve({
            name: file.name,
            size: file.size,
            contentType: getContentTypeOrDefault(file),
  ...
  async function handleStartUpload(): Promise<void> {
    try {
      ...
      const metadataResponse = await fetch('/api/s3-upload-metadata-presign', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
