...
jest.mock("../../domain/lessonProcessing/stepWrapper", () => ({
	createStepWrapper: jest.fn(() => {
		return (stepName: string, handler: any, options?: any) => {
...
jest.mock("../helpers/config", () => ({
	getConfig: jest.fn(() => ({
		...
		EXTRACT_JOB_QUEUE: "https://sqs.ap-northeast-1.amazonaws.com/123456789012/extract-job-queue"
	}))
}));
...
describe("submitExtractTextTask", () => {
	...
	it("should send message to SQS with correct parameters", async () => {
		...
		const { handler } = require("./submitExtractTextTask");
		await handler(event);
	...
	it("should throw error when event is missing required fields", async () => {
		...
		const { handler } = require("./submitExtractTextTask");
		await expect(handler(invalidEvent)).rejects.toThrow("Missing lessonId, userId, or taskToken");
	});

	it("should throw error when event is null", async () => {
		const { handler } = require("./submitExtractTextTask");
		await expect(handler(null)).rejects.toThrow("Missing lessonId, userId, or taskToken");
	});
}); 