...
describe('s3UploadPresignUrlHandler', () => {
  ...
  const loadHandler = async (
    ...
    return (await import('./s3UploadPresignUrl')).handler as (
  ...
  it('should return 200 with presigned URL', async () => {
    const handler = await loadHandler(undefined, {
      getUserUploadUrl: jest.fn().mockResolvedValue('https://example.com'),
    } as any);
    ...
    expect(response.body).toBe(JSON.stringify({ presignUrl: 'https://example.com' }));
  });
