...
const repo = new DynamoLessonProcessingRepository(
  "lesson-processing-steps",
  dynamo
);
...
export const handler = stepWrapper(
  ...
  async (event: { lessonId: string, userId: string, taskToken: string }) => {
    ...
    await sqs.send(new SendMessageCommand({
      QueueUrl: config.EXTRACT_JOB_QUEUE,
      MessageBody: JSON.stringify({ lessonId: event.lessonId, userId: event.userId, taskToken: event.taskToken }),
    }));
  },
  { asyncTask: true }
);